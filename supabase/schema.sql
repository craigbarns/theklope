...
create table if not exists public.orders (
  ...
  checkout_url text,
  payment_create_status text not null default 'not_started',
  payment_create_started_at timestamptz,
  ...
  refund_id text,
  refund_status text,
  refund_attempt integer not null default 0,
  ...
  confirmation_customer_email_sent_at timestamptz,
  confirmation_admin_email_sent_at timestamptz,
  confirmation_email_last_attempt_at timestamptz,
  shipping_email_sent_at timestamptz,
  refund_email_sent_at timestamptz,
  refund_email_last_attempt_at timestamptz,
...
-- Keep this snapshot aligned with 202607210001_harden_checkout_payments.sql.
-- =========================================================================
begin;
...
-- Manual service-role recovery only, after exhaustive provider reconciliation.
-- Automated scans are bounded and must never use them as proof of no duplicate.
